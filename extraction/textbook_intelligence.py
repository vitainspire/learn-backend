...
IMAGES_DIR = PROJECT_ROOT / "static" / "textbook_images"
...
{{
    "entities": {{
        "exercises": [
            {{"id": "E_1_1_1", "text": "Question text...", "topic_id": "T_1_1", "image_id": "IMG_P5_I0"}}
        ],
        "sidebars": [
...
IMPORTANT: Only add "image_id" to an exercise if its text says "look at the picture/figure/image below" or similar AND a matching image ID is listed in the CONTEXT above. Omit "image_id" entirely otherwise.
...
def extract_page_images(doc, page_num: int, book_name: str) -> list[tuple[str, str]]:
    """
    Extract images from a single PDF page using PyMuPDF.
    Saves PNGs to static/textbook_images/{book_name}/ and returns
    a list of (image_id, relative_url) tuples.
    """
    out_dir = IMAGES_DIR / book_name
    ...
    for img_idx, img_info in enumerate(page.get_images(full=True)):
        ...
        try:
            ...
            if pix.width < 40 or pix.height < 40:
                continue
            img_id = f"IMG_P{page_num}_I{img_idx}"
            filename = f"{img_id}.png"
            pix.save(str(out_dir / filename))
            rel_url = f"/static/textbook_images/{book_name}/{filename}"
            results.append((img_id, rel_url))
        except Exception as e:
            print(f"[IMG] Skipping image {img_idx} on page {page_num}: {e}")
    return results
...
def generate_ontology(pdf_path: str, output_dir: str = "output"):
    ...
    # image_map: {img_id -> relative_url} for all extracted images
    image_map: dict[str, str] = {}
    # page_image_ids: {page_num -> [img_id, ...]} for building per-chunk context
    ...
    if not detected_chapters:
        ...
        chunks = [{"title": "Full Book", "text": text, "pages": all_pages}]
    else:
        ...
        for chap in detected_chapters:
            ...
            chunk_pages = list(range(start, end + 1))
            chunk_text = extract_full_text(pdf_path, pages=chunk_pages)
            chunks.append({"title": chap['title'], "text": chunk_text, "pages": chunk_pages})

    # Extract images from every page that belongs to a chunk
    print("[IMG] Extracting page images...")
    for chunk in chunks:
        for page_num in chunk.get("pages", []):
            imgs = extract_page_images(doc, page_num, book_name)
            if imgs:
                page_image_ids[page_num] = [img_id for img_id, _ in imgs]
                for img_id, rel_url in imgs:
                    image_map[img_id] = rel_url
    print(f"[IMG] Extracted {len(image_map)} images total.")
    ...
    for idx, chunk in enumerate(chunks):
        ...
        def run_stage(prompt_template, stage_name, extra_ctx=""):
            print(f"  -> {stage_name}...")
            import time
            max_retries = 5
        ...
        # Build image context for STAGE3: list image IDs available on each page
        image_lines = []
        for page_num in chunk.get("pages", []):
            ids = page_image_ids.get(page_num, [])
            if ids:
                image_lines.append(f"  Page {page_num + 1}: {', '.join(ids)}")
        image_ctx = ""
        if image_lines:
            image_ctx = "IMAGES AVAILABLE (page → image IDs):\n" + "\n".join(image_lines)
        ...
        try:
            ...
            # Stage 3: Exercises (pass image context so AI can map image_id)
            data_s3, raw_s3 = run_stage(STAGE3_PROMPT, "Stage 3: Exercises & Details", extra_ctx=image_ctx)
            # Inject image_url into exercises that have an image_id
            for ex in data_s3.get("entities", {}).get("exercises", []):
                img_id = ex.pop("image_id", None)
                if img_id and img_id in image_map:
                    ex["image_url"] = image_map[img_id]
            merge_data(data_s3)
