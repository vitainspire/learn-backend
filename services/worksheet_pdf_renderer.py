...
from reportlab.platypus import (
    ...
    Table, TableStyle, PageBreak, KeepTogether, Image as RLImage,
)
...
def _add_image(q: dict, story: list):
    """Embed a textbook image when q['image_url'] points to a saved PNG."""
    url = q.get("image_url")
    if not url:
        return
    # Map /static/... URL to absolute file path
    if url.startswith("/static/"):
        img_path = _PROJECT_ROOT / "static" / url[len("/static/"):].lstrip("/")
    else:
        img_path = Path(url)
    ...
    try:
        ...
        story.append(RLImage(str(img_path), width=5 * cm, height=5 * cm, kind="proportional"))
        story.append(Spacer(1, 0.12 * cm))
    except Exception as e:
        print(f"[PDF] Warning: could not embed image {url}: {e}")
