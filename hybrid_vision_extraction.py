...
def call_gemini(contents: list, mime: str = "text/plain", label: str = "") -> str:
    """Call OpenRouter with retry logic (drop-in replacement for the old Gemini call)."""
    ...
    user_content = parts if (has_images or len(parts) > 1) else parts[0]["text"]

    messages = []
    if is_json:
        messages.append({"role": "system", "content": "You must respond with valid JSON only. No markdown fences, no preamble."})
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            print(f"  [OPENROUTER{'+'+label if label else ''}] len={len(text)}")
            return text
